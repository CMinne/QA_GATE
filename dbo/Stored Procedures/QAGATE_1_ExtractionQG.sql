...
(
	...
	totalPiece INT,																					-- Nombre de pièce contrôlée (Par addition)
...
BEGIN
		...
		BEGIN
			...
			IF(@Flag = 1)																			-- Si il y a un changement de jour, alors faire le script
				BEGIN
					...
					IF((SELECT Etat FROM QAGATE_1_EventData WHERE idEvent = @Id_prevF) = 2)			-- Vérifie si on est en run
						BEGIN
							
							SET @Time_Run += DATEDIFF(MINUTE, DATEADD(HOUR, 6, CAST(@Debut_Date AS DATETIME)), @Time2)
																									-- Calcul du temps entre 06:00:00 et le premier évènement du jour
							--SELECT @Time2 AS 'TIME RUN', @Time_Run AS 'RUN TOT'
							--SELECT '1 RUN'
						END
					...
					ELSE																			-- Si on est en stop ou setup
						BEGIN
							...
							SET @Time_Stop += DATEDIFF(MINUTE, DATEADD(HOUR, 6, CAST(@Debut_Date AS DATETIME)), @Time2)
																									-- Calcul du temps entre 06:00:00 et le premier évènement du jour
							--SELECT '1 STOP'
						END
			...
			IF((SELECT timeStamp FROM QAGATE_1_EventData WHERE idEvent = @Id) < DATEADD(HOUR, 30, CAST(@Debut_Date AS DATETIME)))
				BEGIN																				-- Regarde si le prochain évènement est dans le même jour (06:00:00 et 06:00:00 le lendemain)
					...
					IF((SELECT Etat FROM QAGATE_1_EventData WHERE idEvent = @Id_prev) = 2)			-- Vérifie si on est en run 
						BEGIN

							SET @Time_Run += DATEDIFF(MINUTE, @Time1, @Time2)						-- Calcul du temps entre l'évènement précédent et l'évènement actuel
							--SELECT @Time1 AS 'TIME RUN', @Time2 AS 'TIME RUN', @Time_Run AS 'RUN TOT'
							--SELECT '3 RUN'
						END
					...
					ELSE																			-- Si on est en stop ou setup
						BEGIN
							...
							SET @Time_Stop += DATEDIFF(MINUTE, @Time1, @Time2)						-- Calcul du temps entre l'évènement précédent et l'évènement actuel
							--SELECT '3 STOP'
						END
					IF (@Id = @Id_prev)																-- Si on est au dernier évènement
						BREAK																		-- On sort de la boucle WHILE
				...
				BEGIN
					...
					IF((SELECT Etat FROM QAGATE_1_EventData WHERE idEvent = @Id_prev) = 2)			-- Vérifie si on est en run 
						BEGIN
							...
							SET @Time_Run += DATEDIFF(MINUTE, @Time1, DATEADD(HOUR, 30, CAST(@Debut_Date AS DATETIME)))
																									-- Calcul du temps entre l'évènement précédent et 06:00:00 le lendemain
							--SELECT @Time1 AS 'TIME RUN', @Time_Run AS 'RUN TOT'
							--SELECT '4 RUN'
						END
					...
					ELSE																			-- Si on est en stop ou setup
						BEGIN
							...
							SET @Time_Stop += DATEDIFF(MINUTE, @Time1, DATEADD(HOUR, 30, CAST(@Debut_Date AS DATETIME)))
																									-- Calcul du temps entre l'évènement précédent et 06:00:00 le lendemain
							--SELECT '4 STOP'
						END
					...
					SET @Total += (SELECT COUNT(idPiece)											-- Additionne le nombre de pièce contrôlée pour le jour en cours
								   ...
								   AND timeStamp < DATEADD(HOUR, 30, CAST(@Debut_Date AS DATETIME)))
					...
					INSERT INTO #temp_table (currentOF, reference , dateOF     , run      , arret     , totalPiece, keyence , kogame )
						   VALUES			(@OF	  , @Reference, @Debut_Date, @Time_Run, @Time_Stop, @Total    , @Keyence, @Kogame)
	...
	IF((SELECT Etat FROM QAGATE_1_EventData WHERE idEvent = @Id_prev) = 2)
		BEGIN
			...
			IF(@Time2 > @Time1)
				SET @Time_Run += DATEDIFF(MINUTE, @Time1, @Time2)
			--SELECT 'FIN RUN'
		END
	...
	ELSE
		BEGIN
			...
			SELECT TOP(1) @Time2 = timeStamp FROM QAGATE_1_MainTable WHERE currentOF = @OF ORDER BY timeStamp DESC
			IF(@Time2 > @Time1)
				SET @Time_Stop += DATEDIFF(MINUTE, @Time1, @Time2)
			--SELECT 'FIN STOP'
		END
	...
	SELECT currentOF, reference, FORMAT (dateOF, 'dd/MM/yy') AS 'dateJour', run, arret, totalPiece, keyence , kogame 
